...
def get_books():  # function to retrieve the book list from our file.
    ...
    with open('./books.csv', 'r') as books_file:
        # read icontent of the file by line and save to  book_list
        book_list = books_file.readlines()
...
def show_books(books):  # function to display the saved books.
    for index, book in enumerate(books, 1):
        print(
            f"{index}). {book['book_title']} - ({book['release_date']}) by {book['book_author']}")
    print()
